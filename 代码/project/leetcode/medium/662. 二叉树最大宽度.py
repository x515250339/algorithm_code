    ...
    def widthOfBinaryTree(self, root: Optional[TreeNode]) -> int:
        ...
        while arr:
            ...
            for node, index in arr:
                ...
                if node.right:
                    tmp.append([node.right, index * 2 + 1])
            print(arr[-1][1] - arr[0][1] + 1)
            res = max(res, arr[-1][1] - arr[0][1] + 1)
            arr = tmp
        return res


print(Solution().widthOfBinaryTree(makeTree([1, 3, 2, 5, 3, "null", 9])))
print(Solution().widthOfBinaryTree(makeTree([1, 3, 2, 5, "null", "null", 9, 6, "null", 7])))
